/*
  Proyecto: Alarma en tiempo real con DS3231
  Autor: Edgar Cano
  Rev: 13/05/2024   
*/

// include LCD library code
//#include <LiquidCrystal.h>    // Librería para utilizar LCD sin convertidor a i2C 
#include <LiquidCrystal_I2C.h>
// incluir el código de la librería Wire (necesaria para la comunicación por medio del protocolo I2Cs)
#include <Wire.h>
...
const int button1   =  5;                   // pin button1 
const int button2   =  4;                   // pin button2  
const int button3   =  3;                   // pin button3  
const int alarm_pin =  6;                   // pin buzzer  
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(13), Alarm, FALLING);   // Configurando interrupción por flanco de bajada ISR -> Alarm
  lcd.init();
  lcd.backlight();                  
  delay(500);  
}
...
void loop() {
  if(!digitalRead(button1)){                    // Si el botón B1 es presionado
    ...
    if(!digitalRead(button2) && digitalRead(alarm_pin)){         // Cuando el botón B2 es presionado con la alarma activada, se debe restablecer y apagar la alarma.
      digitalWrite(alarm_pin, LOW);               // Apagar el indicador de alarma.
      Wire.beginTransmission(0x68);               // Iniciar el protocolo I2C con la dirección del DS3231.
